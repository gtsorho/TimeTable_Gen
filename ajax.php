...
function generateTimetable($connection, $data)
{
    ...
    foreach ($timeslots as $i => $time) {
        echo "<tr><td><b>{$time['timeslot']}</b></td>";
        foreach ($days as $j => $day) {
            print_r([$dept_id,$sem_id,$time['id'], $day['id']]);
            $result = mysqli_query($connection, "SELECT id FROM timetable where dept_id='{$dept_id}' AND sem_id='{$sem_id}' AND timeslot_id='{$time['id']}' AND day_id='{$day['id']}'");
            $rowcount = mysqli_num_rows($result);
            // echo $rowcount;
            if ($rowcount > 0) {
                $query = "SELECT * FROM timetable  inner join timetable_batches on timetable.id=timetable_batches.timetable_id inner join subject_staff  on subject_staff.id=timetable_batches.subject_staff_id inner join subjects on subject_staff.sub_id=subjects.id inner join staffs on staffs.id=subject_staff.staff_id where timetable.dept_id='{$dept_id}' AND timetable.sem_id='{$sem_id}' AND timetable.timeslot_id='{$time['id']}' AND timetable.day_id='{$day['id']}'";
                $resultset = mysqli_fetch_all(mysqli_query($connection, $query), MYSQLI_ASSOC);
                print_r($resultset);
                echo "<td>";
...
function getSubjects($conn, $data)
{
    ...
    $sem_id = $data['sem_id'];
    $type_id = $data['type_id'];
    // print_r([$type_id,$type_id, $dept_id]);
    $sql = "SELECT * FROM subjects sub  inner join subject_staff ss on sub.id=ss.sub_id inner join staffs on ss.staff_id=staffs.id where sub.dept_id='$dept_id' and sub.sem_id='$sem_id' and ss.type_id='$type_id'";
    $query = mysqli_query($conn, $sql);
    $res = mysqli_fetch_all($query, MYSQLI_ASSOC);
    return json_encode($res);
}
